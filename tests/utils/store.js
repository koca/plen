...
Store.prototype.mockGetter = function(name, stub) {
  ...
  let store = this;
  let mockedGetters = (store.__mockedGetters =
    store.__mockedGetters || new Map());

  if (mockedGetters.has(name)) {
    throw new Error(
      `Cannot mock getter with the name '${name}' twice. Restore the getter or call stub.reset() instead.`
    );
  }
